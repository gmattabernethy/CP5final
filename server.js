...
app.get('/api/log',(req, res)=>{
	knex('users').where('email','glenn.a@att.com').first().then(user => {
	if(user===undefined){
	    res.send("hello");
	}else{
	    res.send("oh");	
...
app.get('/api/purchases', (req, res) => {
  knex('watches').select().from('purchases').then(dbpurchases => {
...
app.post('/api/purchases', (req, res) => {
  ...
  knex('purchases').insert({name:req.body.name, email:req.body.email, address:req.body.address, city:req.body.city, st:req.body.st, zip:req.body.zip, watch:watch, watchName:watchInfo.name, text:req.body.text, price:watchInfo.price, date: new Date()}).then(purchase => {
...
app.delete('/api/purchases/:id', (req, res) => {
  ...
  knex('purchases').where('id',id).del().then(purchases => {
    res.sendStatus(200);    
  }).catch(error => {
    console.log(error);
    res.status(500).json({ error });
...
app.put('/api/purchases/:id', (req, res) => {
  let id = parseInt(req.params.id);
  knex('purchases').where('id',id).update({name:req.body.name}).then(purchases => {
    res.sendStatus(200);    
  }).catch(error => {
    console.log(error);
    res.status(500).json({ error });
